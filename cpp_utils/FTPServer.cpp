...
std::string FTPServer::listenPassive() {
	...
	long unsigned int addrInfoSize = sizeof(clientAddrInfo);
	...
	long unsigned int addrLen = sizeof(serverAddress);
...
void FTPServer::receiveFile(std::string fileName) {
	...
	ESP_LOGD(LOG_TAG, "<< receiveFile: totalSizeRead=%lu", totalSizeRead);
...
void FTPServer::sendData(uint8_t* pData, uint32_t size) {
	ESP_LOGD(LOG_TAG, ">> FTPServer::sendData: size=%lu", size);
...
int FTPServer::waitForFTPClient() {
	...
	long unsigned int socketAddressInfoSize = sizeof(socketAddressInfo);

...
BLE::BLE(){this->servername = "ESP32_BLE_R";}
BLE::BLE(const std::string& servername){this->servername = servername;}
BLE::~BLE(){this->stopAdvertising();}
void BLE::init(){BLEDevice::init(this->servername);}
...
void BLE::startAdvertising(){this->pServer->getAdvertising()->start();}
void BLE::stopAdvertising(){this->pServer->getAdvertising()->stop();}
...
void BLE::onConnect(BLEServer *pServer, BLEConnInfo &connInfo) {this->deviceConnected = true;};
...
void BLE::onAuthenticationComplete(BLEConnInfo& connInfo){printf("Starting BLE work!\n");}
...
void BLE::onWrite(BLECharacteristic *pCharacteristic, BLEConnInfo &connInfo){
    ...
    if(rxValue.contains("Comando") && this->deviceConnected) {this->enviar = true;}
}